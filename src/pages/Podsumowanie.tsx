...
export default function Podsumowanie() {
  ...
  const getSymptomLabel = (symptom: string) => {
    const labels: Record<string, string> = {
      ...
      low_mood_stress: "Obniżony nastrój",
      depressive_symptoms: "Objawy depresyjne",
      mental_exhaustion: "Wyczerpanie psychiczne",
      emotional_crisis: "Kryzys emocjonalny",
      daily_functioning_difficulty: "Trudność w codziennym funkcjonowaniu",
      lack_of_energy: "Poczucie braku sił",
      mental_tension: "Napięcie psychiczne",
      feeling_overwhelmed: "Poczucie przeciążenia",
      resignation_thoughts: "Myśli rezygnacyjne",
      feeling_threatened: "Uczucie zagrożenia",
      chest_pressure: "Ucisk w klatce piersiowej",
      heart_palpitations: "Kołatanie serca",
      anger_outbursts: "Wybuchy złości",
      frequent_waking: "Częste wybudzanie się",
      daytime_sleepiness: "Senność w ciągu dnia",
      nightmares: "Koszmary senne",
      suicidal_thoughts: "Myśli samobójcze",
        ...
        <div className="space-y-6">
          ...
          {isCareLeave && (
            <Card className="border-primary/30 bg-primary/5">
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{isChildCare ? "PESEL dziecka:" : "PESEL:"}</span>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {formData.rodzajZwolnienia?.leave_type === "pl_employer" &&
                formData.rodzajZwolnienia?.nips?.length > 0 && (
                  <div className="flex justify-between">
              ...
              {/* NIP - care_family */}
              {formData.rodzajZwolnienia?.leave_type === "care_family" &&
                formData.rodzajZwolnienia?.care_family_nips?.length > 0 && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">NIP pracodawcy:</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{isCareLeave ? `Wywiad medyczny ${getPatientLabel()}` : "Wywiad medyczny"}</CardTitle>
            </CardHeader>
              ...
              <div className="space-y-2 pt-4 border-t">
                ...
                {formData.wywiadObjawy?.symptom_duration && (
                  <div>
                    ...
                    <span className="font-medium">{getDurationLabel(formData.wywiadObjawy.symptom_duration)}</span>
                  </div>
                )}
